...
def train(args):
  ...
  x_train, y_train = utils.condense_data(args.data_dir + args.train_file, args.adjust_sentiment)
  ...
  if os.path.exists(model_file):
    ...
    if 'step' in best_point:
      step = best_point['step'] + 1

  eval_at = args.evaluate_every
  stop_at = args.training_stopper
  
  print('Start training...')
  for epoch in trange(step, args.num_epoch):
    if stop_at <= 0:
      break
...
def create_synthetic_data(args):
  ...
  file_name = args.data_dir + '%s/train.plan.jsonl' % args.data_type
  ...
  x_train, _ = utils.abstract_data(args.data_dir + args.train_file)
...
if __name__ == '__main__':
  ...
  parser.add_argument('-num_epoch', default=1, type=int) # 30
  parser.add_argument('-batch_size', default=1, type=int) # 16
  parser.add_argument('-learning_rate', default=3e-5, type=float)

  parser.add_argument('-warmup', default=8_000, type=int) # 8_000
  parser.add_argument('-evaluate_every', default=20_000, type=int) # 20_000
  parser.add_argument('-training_stopper', default=50, type=int)

  parser.add_argument('-train_file', default='dataset/train.plansum.jsonl', type=str)
  ...
  parser.add_argument('-max_length', default=300, type=int) # 10 (rotten), 90 (yelp), 1000 (amazon)
