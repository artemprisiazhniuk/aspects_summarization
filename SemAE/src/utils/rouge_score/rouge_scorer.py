...
class RougeScorer(scoring.BaseScorer):
    ...
    def __init__(self, language, rouge_types, use_stemmer=False):
        ...
        Args:
          language: Handling language input.
        ...
        self._stemmer = stemmers.stemmers.get(language, None) if use_stemmer else None
    ...
    def score(self, target, prediction):
        ...
        target_tokens = tokenize.tokenize(target, self.language, self._stemmer)
        prediction_tokens = tokenize.tokenize(prediction, self.language, self._stemmer)
        result = {}
        ...
        for rouge_type in self.rouge_types:
            if rouge_type == "rougeL":
                ...
                target_tokens_list = [
                    tokenize.tokenize(s, self.language, self._stemmer) for s in get_sents(target)]
                prediction_tokens_list = [
                    tokenize.tokenize(s, self.language, self._stemmer) for s in get_sents(prediction)]
