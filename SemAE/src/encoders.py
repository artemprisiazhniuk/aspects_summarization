...
# Implementation of the encoder has been
...
class TransformerDocumentEncoder(nn.Module):
    ...
    def __init__(self,
                 d_model=320,
                 sentence_nlayers=3,
                 sentence_internal_nheads=4,
                 sentence_d_ff=512,
                 sentence_output_nheads=8,
                 dropout=0.1):
        '''Parameters:
        ...
        transformer_layer = nn.TransformerEncoderLayer(
            d_model,
            sentence_internal_nheads,
            dim_feedforward=sentence_d_ff,
            dropout=dropout)
        ...
        if sentence_output_nheads > 1:
            self.final_linear = nn.Linear(d_model // sentence_output_nheads,
                                          d_model)
    ...
    def forward(self, inputs, padding_mask=None):
        ...
        token_vecs = self.transformer_encoder(
            inputs, src_key_padding_mask=padding_mask)
        token_vecs = token_vecs.transpose(0,
                                          1).reshape(batch_size, nsent, -1,
                                                     self.d_model)
...
class PositionalDocumentEncoding(PositionalEncoding):
    ...
    def forward(self, emb):
        ...
        emb = emb.reshape(batch_size * nsent, ntokens,
                          emb_size).transpose(0, 1)
        emb = super(PositionalDocumentEncoding, self).forward(emb)
        return emb.transpose(0, 1).reshape(batch_size, nsent, ntokens,
                                           emb_size)
