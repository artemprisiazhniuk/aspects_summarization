...
def create_train_data(args,
  ...
  f = open(args.data_dir + '/' + args.dataset + '/train.acesum.jsonl', 'r', encoding='utf-8')
  for line in tqdm(f):
  ...
  dataset_file = args.data_dir + '/' + args.dataset + '/train.sum.jsonl'
  ...
  for inst in tqdm(data):
    ...
    for i in range(0, len(total_reviews), 1000):
      ...
      for s_id, summary in enumerate(reviews):
        ...
        f = open(dataset_file, 'a', encoding='utf-8')
...
def create_general_test_data(args,
  ...
  for split in ['dev', 'test']:
    ...
    f = open(args.data_dir + '/' + args.dataset + '/' + split + '.json', 'r', encoding='utf-8')
    data = json.load(f)
    f.close()

    f = open(args.data_dir + '/' + args.dataset + '/' + split + '.sum.general.jsonl', 'w', encoding='utf-8')
...
if __name__ == '__main__':
  ...
  parser.add_argument('-model_type', default='DeepPavlov/rubert-base-cased', type=str) # distilroberta-base
