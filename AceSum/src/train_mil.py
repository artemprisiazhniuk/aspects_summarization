...
def evaluate(args):
  ...
  aspects = 'здание чистота еда расположение номер обслуживание'.split()
  ...
  for inp_batch, _ in tqdm(dataloader):
    model.eval()
  ...
  while step < args.no_train_steps:
    ...
    for _, (inp_batch, out_batch) in enumerate(tqdm(dataloader)):
      ...
      if step % args.ckpt_every == 0:
        ...
        }, args.model_dir + '/' + args.dataset + '/' + args.model_name + '.model')
        losses = []
        print('-' * 50)
        print(f'step: {step}\nloss:{np.mean(losses)}\ndoc_f1:{doc_f1}\nsent_f1:{sent_f1}\n')
...
if __name__ == '__main__':
  ...
  parser.add_argument('-batch_size', default=4, type=int) # 32
  parser.add_argument('-learning_rate', default=1e-4, type=float)
  parser.add_argument('-no_train_steps', default=10_000, type=int) # 10_000
  parser.add_argument('-no_warmup_steps', default=10_000, type=int) # 10_000
