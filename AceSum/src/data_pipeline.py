...
class FinetuneDataset(IterableDataset):
  ...
  def process(self, inst, file_idx):
    ...
    inp = ' '.join(['<rev> ' + ' '.join(review['sentences']) for review in reviews]).lower()
...
class SummarizationDataset(IterableDataset):
  ...
  def __iter__(self):
    if self.shuffle:
      dataset_iters = [open(file, 'r', encoding='utf-8') for file in self.files]
      shufbuf = []
      file_indices = []
      try:
        ...
        while True:
          for i, dataset_iter in enumerate(dataset_iters):
            try:
              ...
              dataset_iters[i] = open(self.files[i], 'r', encoding='utf-8')
        #while len(shufbuf) > 0:
...
class AspectDetectionDataset(IterableDataset):
  ...
  def __iter__(self):
    if self.shuffle:
      shufbuf = []
      try:
        dataset_iter = open(self.file, 'r', encoding='utf-8')
