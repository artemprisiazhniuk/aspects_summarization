...
def get_vocab(filedir, min_len, max_len, max_seg, stop_words, lemmatize):
    """
    ...
    f_domain = open(filedir + '/train.jsonl', 'r')
...
def load_data(filedir, args):
    ...
    max_len_actual, max_seg_actual, seg_cnt, doc_cnt, word2id = get_vocab(filedir,
    ...
    for inst, docs, lbls, orig in zip(insts, data, data_lbl, data_orig):
        reviews = inst['reviews'] # reviews for one product
        for review in reviews:
            sentences = review['sentences'] # separate sentences of one review
...
def main():
    ...
    # loads data
    word2id, all_docs, all_label, all_original = load_data('./data/space', args)
    ...
    # find top-5 close words and aspect names
    nn = NearestNeighbors(n_neighbors=5).fit(w_emb_array)
    ...
    for k in tqdm(range(args.aspects)):
        ...
        f = open(f'./data/preprocessed/{aspect_name}.txt', 'w')
        print(*sw, sep='\n', end='', file=f)
        f.close()
