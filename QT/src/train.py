...
if __name__ == '__main__':
    ...
    data_arg_group.add_argument('--data', 
            ...
            type=str, default='../data/dataset/train.jsonl')
    ...
    other_arg_group.add_argument('--checkpoint',
            type=str, default=None)
    other_arg_group.add_argument('--finetune',
            type=bool, default=False)
    other_arg_group.add_argument('--use_wandb',
            action='store_true')
    ...
    if args.use_wandb:
        wandb.login()
        wandb.init(project='QT', name='train', config=vars(args))
    ...
    for epoch in trange(start_epoch, args.epochs):
        ...
        if args.save_every is not None and epoch % args.save_every == args.save_every - 1:
            ...
            torch.save({
                ...
                'start_epoch': epoch
            }, '{0}/{1}_{2}_checkpoint.pt'.format(save_path, args.run_id, epoch+1))

            if args.use_wandb:
                wandb.save('{0}/{1}_{2}_checkpoint.pt'.format("models", args.run_id, epoch+1))
